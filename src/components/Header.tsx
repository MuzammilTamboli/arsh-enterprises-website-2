...
const Header = () => {
  ...
  const navItems = ['Home', 'Services', 'About Us', 'Pricing', 'Testimonials', 'Contact Us'];
            ...
            <div className="px-2 pt-2 pb-3 space-y-1">
              {navItems.map((item) => (
                <a
                  key={item}
                  href={`#${item.toLowerCase().replace(' ', '-')}`}
                  className="block px-3 py-2 text-gray-700 hover:text-sky-600 hover:bg-sky-50 rounded-md transition-colors duration-300"
